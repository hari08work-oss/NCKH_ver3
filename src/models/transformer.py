...
#     - Segmentation head: xuất mask
#     - Classification head: xuất nhãn bệnh (multi-class)
#     """
#     def __init__(self, in_channels=1, out_channels=1, num_classes=3,
#                  img_size=256, patch_size=16, emb_size=256, depth=4, num_heads=4):
#         super().__init__()
#         self.embed = PatchEmbedding(in_channels, patch_size, emb_size, img_size)
#         self.encoder = nn.ModuleList(
#             [TransformerBlock(emb_size=emb_size, num_heads=num_heads)
#              for _ in range(depth)]
#         )

#         # Decoder để xuất segmentation mask
#         self.decoder = nn.Sequential(
#             nn.ConvTranspose2d(emb_size, 256, 2, stride=2),
#             nn.ReLU(),
...
#         # Classification head (dùng feature trung bình toàn cục)
#         self.classifier = nn.Sequential(
#             nn.AdaptiveAvgPool2d((1, 1)),   # [B, emb, h, w] -> [B, emb, 1, 1]
#             nn.Flatten(),
...
#         x = x[:, 1:, :]
#         h = w = self.img_size // self.patch_size
#         features = x.transpose(1, 2).reshape(B, self.emb_size, h, w)
...
#     seg, cls = model(x)
#     print("Input:", x.shape)
#     print("Segmentation Output:", seg.shape)  # [B, 1, 256, 256]
#     print("Classification Output:", cls.shape)  # [B, num_classes]
