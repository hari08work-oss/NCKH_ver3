...
def map_vector_to_config(vec):
    ...
    emb     = [128, 192, 256, 384, 512][clamp(to_int(np.interp(vec[4], [0, 1], [0, 4])), 0, 4)]
    patch   = [8, 16][clamp(to_int(np.interp(vec[5], [0, 1], [0, 1])), 0, 1)]
    loss_w  = np.interp(vec[6], [0, 1], [0.0, 1.0])  # multi-task weight (nếu dùng)
    img_sz  = [256, 512][clamp(to_int(np.interp(vec[7], [0, 1], [0, 1])), 0, 1)]  # 256 hoặc 512
...
def quick_eval(cfg, train_subset_ratio=0.25, proxy_epochs=5, seed=42):
    ...
    device = DEVICE
    img_sz = cfg["img_size"]  # sẽ là 512

    # loader
    full_train = get_loader(DATA_DIR + "/train", MASK_DIR + "/train",
    ...
    for ep in range(proxy_epochs):
        ...
        for imgs, masks, _ in sub_batches:
            imgs = torch.nn.functional.interpolate(imgs, size=(img_sz, img_sz),
                                                   mode="bilinear", align_corners=False)
            masks = torch.nn.functional.interpolate(masks, size=(img_sz, img_sz),
                                                    mode="nearest")
            imgs, masks = imgs.to(device), masks.to(device)

            preds = model(imgs)
            # resize preds về đúng size mask nếu mismatch
        ...
        print(f"[Proxy] Epoch {ep+1}/{proxy_epochs} "
              f"Loss={ep_loss/len(sub_batches):.4f}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--subset", type=float, default=0.25, help="Tỉ lệ subset train cho proxy")
